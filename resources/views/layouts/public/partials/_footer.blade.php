    <div class="style-1">
        <div class="container">
            <footer id="footer">
                <div class="grid footer_header">
                    <div class="grid__item grid__item--md-span-4 footer_header_item">
                        <h5 class="header_underline">Páginas</h5>
                        <ul>
                            <li><a href="#">Início</a></li>
                            <li><a href="#">Sobre</a></li>
                            <li><a href="#">Portfólio</a></li>
                            <li><a href="#">Contactos</a></li>
                        </ul>
                    </div>
                    <div class="grid__item grid__item--md-span-4 footer_header_item">
                        <h5 class="header_underline">Redes Sociais</h5>
                        <a href="#"><i class="fab fa-facebook-f"></i>acebook</a>
                    </div>
                    <div class="grid__item grid__item--md-span-4 footer_header_item">
                        <h5 class="header_underline">Hugo Casqueiro</h5>
                        <ul>
                            <li><i class="fas fa-envelope"></i> email@teste.pt</li>
                            <li><i class="fas fa-phone"></i> +351 919 123 654</li>
                            <li><i class="fas fa-address-card"></i> Rua dos Moinhos, nº54</li>
                        </ul>
                    </div>
                </div>
                <div class="grid footer_footer">
                    <div class="grid__item grid__item--md-span-12">
                        <span>Copyright © <?php echo date("Y"); ?> Hugo Casqueiro by <a href="https://www.linkedin.com/in/andsanalmeida/"target="_blank" title="LinkedIn"><b><u>André Almeida</u></b></a></span>
                    </div>
                </div>
            </footer>
        </div>       
    </div>    

    <!-- JAVASCRIPTS -->
    <script src="https://code.jquery.com/jquery-3.3.1.slim.min.js"></script>
    <script src="{!! asset('js/main.js') !!}" type="text/javascript" charset="utf-8" async defer></script>    
</html>